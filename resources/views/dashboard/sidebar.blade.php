<!-- Sidebar Toggle -->
<div class="sticky top-0 bg-stone-800 inset-x-0 z-10 border-b border-stone-700 px-4 sm:px-6 md:px-8 lg:hidden">
    <div class="flex items-center py-4">
        <!-- Navigation Toggle -->
        <button id="sidebar-toggle" class="text-gray-500 hover:text-gray-400 focus:outline-none focus:text-gray-400" aria-label="Toggle sidebar">
        ...
        <ol class="ml-3 flex items-center whitespace-nowrap min-w-0" aria-label="Breadcrumb">
            <a href="/dashboard" class="flex items-center text-sm text-stone-50 dark:text-gray-400">
                Dashboard
            </a>
            <svg class="flex-shrink-0 mx-3 overflow-visible h-2.5 w-2.5 text-gray-400 dark:text-stone-50" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M5 1L10.6869 7.16086C10.8637 7.35239 10.8637 7.64761 10.6869 7.83914L5 14" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
            </svg>
            <li class="text-sm font-semibold text-stone-50 truncate" aria-current="page">
                {{ $title }}
            </li>
        </ol>
        <!-- End Breadcrumb -->
...
<div id="logo-sidebar" class="-translate-x-full transition-all duration-300 transform fixed top-0 left-0 bottom-0 z-10 w-64 bg-stone-800 border-r border-stone-700 pt-7 pb-10 overflow-y-auto scrollbar-y lg:block lg:translate-x-0 lg:right-auto lg:bottom-0">
    ...
    <nav class="hs-accordion-group p-6 w-full flex flex-col flex-wrap">
        <ul class="space-y-1.5">
            <li>
                <a class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm rounded-md text-stone-50 hover:bg-stone-700 {{ request()->routeIs('dashboard.index') ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}" href="/dashboard">
            ...
            <details class="group [&_summary::-webkit-details-marker]:hidden">
                <summary class="flex cursor-pointer items-center justify-between rounded-lg px-2.5 py-2 text-stone-50 hover:bg-stone-700 {{ in_array($title, ['Pertemuan', 'Tugas', 'Materi']) ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}">
                ...
                <ul class="mt-2 space-y-1 px-4">
                    <li>
                        <a href="/dashboard/pertemuan" class="block rounded-lg px-6 py-2 text-sm font-medium text-stone-50 hover:bg-stone-700 {{ $title === 'Pertemuan' ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}">
                        Pertemuan
                        </a>
                    </li>
                    <li>
                        <a href="/dashboard/materis" class="block rounded-lg px-6 py-2 text-sm font-medium text-stone-50 hover:bg-stone-700 {{ $title === 'Materi' ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}">
                        Materi
                        </a>
                    </li>
                    <li>
                        <a href="/dashboard/tugas" class="block rounded-lg px-6 py-2 text-sm font-medium text-stone-50 hover:bg-stone-700 {{ $title === 'Tugas' ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}">
                        Tugas
                        </a>
                    </li>
                </ul>
            ...
            <details class="group [&_summary::-webkit-details-marker]:hidden">
                <summary class="flex cursor-pointer items-center justify-between rounded-lg px-2.5 py-2 text-stone-50 hover:bg-stone-700 {{ in_array($title, ['Kategori Tes', 'Tambah Kategori Tes', 'PreTest', 'Tambah Soal Pretest', 'Tambah Soal Posttest', 'Tes Harian', 'PostTest', 'Edit Soal Pretest', 'Edit Soal Posttest', 'Import Soal Pretest', 'Import Soal Posttest']) ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}">
                    <div class="flex gap-x-3.5">
                ...
                <ul class="mt-2 space-y-1 px-4">
                    <li>
                        <a href="/dashboard/kategori-tes" class="block rounded-lg px-6 py-2 text-sm font-medium text-stone-50 hover:bg-stone-700 {{ in_array($title, ['Kategori Tes', 'Tambah Kategori Tes', 'Edit Kategori Tes']) ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}">
                        Kategori Tes
                        </a>
                    </li>
                    <li>
                        <a href="/dashboard/pretest" class="block rounded-lg px-6 py-2 text-sm font-medium text-stone-50 hover:bg-stone-700  {{ in_array($title, ['PreTest', 'Tambah Soal Pretest', 'Edit Soal Pretest', 'Import Soal Pretest']) ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}">
                        PreTest
                        </a>
                    </li>
                    <li>
                        <a href="/dashboard/posttest" class="block rounded-lg px-6 py-2 text-sm font-medium text-stone-50 hover:bg-stone-700 {{ in_array($title, ['PostTest', 'Tambah Soal Posttest', 'Edit Soal Posttest']) ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}">
                        PostTest
                        </a>
                    </li>
                    <li>
                        <a href="/dashboard/#" class="block rounded-lg px-6 py-2 text-sm font-medium text-stone-50 hover:bg-stone-700">
                        Harian
                        </a>
                    </li>
                </ul>
            ...
            <li>
                <a class="rounded-xl flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-stone-50 hover:bg-stone-700 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300" href="javascript:;">
            ...
            <li>
                <a class="rounded-xl flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-stone-50 hover:bg-stone-700 {{ $title === 'Siswa' ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}" href="/dashboard/siswa">
            ...
            <li>
                <a class="rounded-xl flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-stone-50 hover:bg-stone-700 {{ $title === 'Kelompok' ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}" href="/dashboard/kelompok">
            ...
            <li>
                <a class="rounded-xl flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-stone-50 hover:bg-stone-700 {{ $title === 'Nilai' ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}" href="/dashboard/nilai">
            ...
            <li>
                <a class="rounded-xl flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-stone-50 hover:bg-stone-700 {{ $title === 'Lencana' ? 'bg-violet-700 text-stone-50 hover:bg-violet-700' : '' }}" href="/dashboard/lencana">
            ...
            <form action="/logout" method="post">
                @csrf
                <button type="submit" class="flex w-full items-center gap-x-3.5 py-2 px-2.5 text-sm text-stone-50 rounded-md hover:bg-stone-700">
