    ...
    @else
        <ul class="mt-4">
            @foreach($results as $result)
                <div class="relative break-words bg-stone-50 border shadow-lg rounded-sm border-b-4 border-r-4 border-stone-700 my-2 p-5">
                    <div class="flex-none">
                        <h2 class="text-stone-400 font-normal text-md text-right ">Materi</h2>
                        <a href="{{ route('student.materi.show', $result->slug) }}" class="text-stone-900 font-semibold text-lg hover:underline">
                            {{ $result->title }}
                        </a>
