...
<section class="my-24 mx-auto justify-center items-center max-w-lg">
    <main class="flex justify-center px-8 py-8 sm:px-12 bg-stone-50 mx-2 border border-r-4 border-b-4 border-stone-700">
        <div class="py-4">
            ...
            <h1 class="clashdisplaymedium text-center mb-6 text-2xl text-stone-900 sm:text-2xl md:text-3xl">Selamat Bergabung 🎉</h1>
            ...
            <form method="POST" action="{{ route('daftar') }}">
                ...
                <div class="col-span-6 sm:col-span-3">
                    <label for="FirstName" class="block text-sm font-medium text-stone-700">
                        Nama Lengkap
                    </label>
                    <input type="text" id="input-name" name="name" class="my-2 p-2 border border-stone-700 rounded-lg w-full" value="{{ old('name') }}" required>
                    @error('name')
                ...
                <div class="col-span-6">
                    ...
                    <input type="email" id="input-email" name="email" class="my-2 p-2 border border-stone-700 rounded-lg w-full" value="{{ old('email') }}" required>
                    @error('email')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-span-6 sm:col-span-3">
                    ...
                    <input type="password" id="input-password" name="password" class="my-2 p-2 border border-stone-700 rounded-lg w-full" value="{{ old('password') }}" required>
                    @error('password')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-span-6 sm:col-span-3">
                    <label for="PasswordConfirmation" class="block text-sm font-medium text-stone-700">
                        Konfirmasi Kata Sandi
                    </label>
                    <input type="password" id="input-password_confirmation" name="password_confirmation" class="my-2 p-2 border border-stone-700 rounded-lg w-full" value="{{ old('password_confirmation') }}" required>
                ...
                <div class="form-actions my-6">
                    <button type="submit" class="clashdisplaymedium w-full relative inline-flex items-center justify-center p-4 px-6 py-3 overflow-hidden text-stone-50 bg-violet-500 transition duration-300 ease-out border border-b-4 border-r-4 border-stone-700 group">
                        <span class="absolute inset-0 flex items-center justify-center w-full h-full text-stone-50 duration-300 -translate-x-full bg-violet-500 group-hover:translate-x-0 ease">
                            <svg fill="currentColor" width="20px" height="20px" viewBox="0 0 16 16" id="register-16px" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span class="absolute flex items-center justify-center w-full h-full transition-all duration-300 transform group-hover:translate-x-full ease">Daftar</span>
                        <span class="relative invisible">Daftar</span>
                    </button>
                </div>

                <p class="mt-6 text-sm text-stone-500 sm:mt-0">
                    Sudah punya akun?
                    <a href="/login" class="clashdisplaymedium text-stone-700 hover:underline">Log in</a>.
                </p>
            </form>
        </div>
    </main>
